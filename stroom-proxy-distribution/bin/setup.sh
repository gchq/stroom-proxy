...
if [ ! -f "bin/setup-${mode}.xml" ]; then
	stroom-echo "Unknown mode in arg ${mode}"
	exit 1
fi   
...
cp -r template-${mode}/* instance/
mkdir instance/temp 

if [ ! -f bin/~setup.xml ]; then
	cp bin/setup-${mode}.xml bin/~setup.xml
fi
...
if [ -n "$IN_DOCKER" ];
then 
	java -cp "lib/*" stroom.proxy.util.config.Configure parameterFile=bin/~setup.xml readParameter=false processFile=instance/conf/server.xml,instance/lib/stroom.properties,bin/~env.sh;
else 
	java -cp "lib/*" stroom.proxy.util.config.Configure parameterFile=bin/~setup.xml processFile=instance/conf/server.xml,instance/lib/stroom.properties,bin/~env.sh;
fi
...
cd lib
DEPLOY_WAR=`ls -1 ${WAR_PREFIX}*.war | java -cp "*" stroom.proxy.util.config.VersionSort | tail -1`
cd ${DIR}/..
...
echo "<ResourceLink global='jdbc/ProxyDB' name='jdbc/ProxyDB' type='javax.sql.DataSource'/>" >> ${CATALINA_BASE}/conf/Catalina/localhost/stroom.xml
